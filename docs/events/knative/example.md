...
In this example we will generate an event in Kafka, consume it in Direktiv via Knative and publish an new event back to Knative.
...
```console
kc get pods -n knative-eventing
...
In Knative there are different broker and channel combinations available. Here we will use Kafka as a Broker. To install Kafka as Knative broker there needs to be a Kafka broker controller and the implementation.
...
```console
kc get brokers.eventing.knative.dev
...
Creating a Kafka source requires to provide the boostrap server and the topic to subscribe to. The topic *knative-direktiv-topic* was created at the beginning of this example and will be the channel to ingest events into Direktiv. The *sink* value configures this source to send all events coming from this channel to the Kafka broker.
...
After running the pod add JSON into the command prompt, e.g. *{}*. This sendd the JSON object to Kafka. Knative's broker will pick it up and executes the trigger for direktiv. The event will appear on the namespace dashboard.
